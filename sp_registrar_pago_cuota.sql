...
BEGIN
    ...
    -- 1. Obtener datos y Bloquear filas (FOR UPDATE)
    SELECT
        id_venta,
        saldo_pendiente
    INTO
    ...
    SELECT
        ...
        estado
    INTO
        v_monto_cuota_esperado,
        v_estado_cuota_actual 
    FROM plan_pagos
    ...
    ELSEIF v_estado_cuota_actual = 'PAGADA' THEN
        ...
        SELECT
            0 AS ok,
            400 AS status,
            'Error: Esta cuota ya se encuentra cancelada' AS msg;
    ELSE
        ...
        INSERT INTO abonos_credito (
            ...
            p_metodo_pago,
            p_comprobante,
            IFNULL(p_fecha_pago, NOW())
        );

        -- 4. Actualización del Plan de Pagos (Estado exacto: PAGADA o PARCIAL)
        UPDATE plan_pagos 
        SET estado = IF(p_monto_pagado >= v_monto_cuota_esperado, 'PAGADA', 'PARCIAL'),
            fecha_pago_real = IFNULL(p_fecha_pago, NOW())
        WHERE id_cuota = p_id_cuota;

        -- 5. Actualización de la Cuenta por Cobrar (Estado exacto: CANCELADA o VIGENTE)
        ...
        SET saldo_pendiente = v_nuevo_saldo_cxc,
            ...
            fecha_creacion = fecha_creacion -- Mantener fecha original
        WHERE id_cxc = p_id_cxc;

        -- 6. Sincronización con Venta (Estado exacto: PAGADA)
        IF v_nuevo_saldo_cxc <= 0 THEN
            UPDATE ventas 
        ...
        SELECT 
            1 AS ok, 
            200 AS status, 
            'Pago registrado correctamente' AS msg, 
